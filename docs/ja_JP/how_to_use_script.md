...
* * ```0```は立ち絵の位置を表します。名前付き位置（```vl```最左、```sl```やや左、```m```中央、```sr```やや右、```vr```最右、旧形式-2、-1、0、1、2と互換）に加え、```(x,y)```座標の直接入力もサポートしています。
...
* * ```@move rar|z1|-2|nod|3|4200```：```@move```はアニメーションを表示する識別子です。**縦棒**```|```で各部分を分割します：

* * ```rar```はキャラクター名です。

* * ```z1```は立ち絵の遠近関係（zオーダー）です。

* * ```-2```は立ち絵の現在位置を表します（```@fg```の位置パラメータと同じ形式）。

* * ```nod```はうなずく動作を表します。他にも```tox```動作があります（xは目標位置で、立ち絵を指定位置に移動させます。例：```to2```で位置2に移動）。
...
* * ```@clear rir```：```@clear```は立ち絵を消去する識別子です。その後ろにキャラクター名を指定し、そのキャラクターの立ち絵を消去します。

* * ```@clear All```：すべての立ち絵を消去します。
