...
* * ```@bg bg1|0.1|0.2|1.5```：其中```@bg```为播放背景图片的标识符，用**竖线**```|```分割各个部分：

* * ```bg1```为用作背景图片的png图片文件，该文件应该被放置在根目录的```source/background```文件夹下,

* * ```0.1|0.2|1.5```分别是x，y，缩放倍数，用于自定义背景的图片位置与大小，可**同时为空**（```|```**可删除**）。

* * 也可以使用```@cg```，功能一致，使用cg的背景图需要被记录进cg鉴赏以及配置文件，具体的使用请查阅[配置文件文档](how_to_use_config.md)
...
* * ```0```代表立绘位置，支持命名位置（```vl```最左、```sl```偏左、```m```居中、```sr```偏右、```vr```最右，兼容旧格式-2、-1、0、1、2），也支持直接输入```(x,y)```坐标，
...
* * ```@move rar|z1|-2|nod|3|4200```：其中```@move```为显示动画的标识符，用**竖线**```|```分割各个部分：

* * ```rar```是角色名字，

* * ```z1```是立绘的远近关系，

* * ```-2```代表立绘的当前位置（与```@fg```的位置参数格式一致），

* * ```nod```代表点头动作，除此之外还有```tox```动作（x是目标位置，可以使立绘移动到指定位置，比如```to2```移动到位置2），
...
* * ```@clear rir```：其中```@clear```为清除立绘的标识符，后面是角色名字，用于清除该角色的立绘。
