...
```@fg rir|z1|rir_z1b0200|b0059|0```：其中```@fg```为显示立绘的标识符，用**竖|线**分割各个部分，其中```rir```是角色名字,```z1```是立绘的远近关系，```rir_z1b0200```是立绘身体的png图片文件，```b0059```是立绘表情的png图片文件，```0```代表立绘位置（从左到右，分别为-2，-1，0，1，2）。立绘身体标识可以**为空**，此时只改变表情，不改变身体。
